...
LeoWinston.prototype._rebuildPropagationChains = function(){
    ...
    this._propagationChains = _.flowRight(_.fromPairs, _.map)(
        _.omit(this.container.loggers, 'root'), // root logger is the topmost
        function(logger, loggerName){
            // Non-propagate loggers don't have a chain
            if (!self.get(loggerName)._leo.options.propagate)
                return [loggerName, []];
            ...
            var proceed = true;
            chain = _.takeWhile(chain, function(name){
                if (proceed){
